...
## Corpus Check
- 290 files · ~134,388 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2296 nodes · 5194 edges · 119 communities (106 shown, 13 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `433dd431`
...
Cohesion: 0.05
Nodes (55): AddAttachmentDialog(), Props, AddLinkDialog(), Props, AddResourceDialog(), Props, Props, AddStatusReportDialog() (+47 more)
...
Cohesion: 0.10
Nodes (37): CategorySelect(), Props, EditProjectDialog(), FY_YEARS, personFromProfile(), Props, PersonAutocomplete(), Props (+29 more)
...
Cohesion: 0.13
Nodes (21): App(), reportsApi, AuthProvider(), usePageTitle(), CodeTablesPage(), labelFor(), CategoriesPage(), CreateProjectWizard() (+13 more)
...
Cohesion: 0.07
Nodes (38): collection, compilerOptions, deleteOutDir, plugins, $schema, sourceRoot, Invalid, LookupOption (+30 more)
...
Cohesion: 0.19
Nodes (7): MilestoneProgressRow, Project, ProjectDetail, ProjectListRow, ProjectListStats, ProjectsRepository, Injectable
...
Cohesion: 0.05
Nodes (57): Props, Props, CommandPalette(), Entry, hitPath(), KIND_META, Props, formatSize() (+49 more)
...
Cohesion: 0.10
Nodes (19): 1. Ground rules (every session obeys these), 2026-08-13 — Phase 0a, frontend test runner (no skill; a port, not a design pass), 2026-08-13 — Phase 0b, characterization tests for the three pilot modules, 2026-08-13 — Phase 1a, `modules/links/` (the control case), 2. Per-session protocol, 3. Skill mapping, 4. Phases, 5. Deliberately out of scope (+11 more)
...
Cohesion: 0.21
Nodes (11): Skeleton(), statusReportsApi, AttachmentDetailPage(), formatSize(), longDate(), relativeTime(), uploaderName(), ACCESS_LABELS (+3 more)
...
Cohesion: 0.10
Nodes (25): AddActionItemDialog(), emptyOwner(), ownerFromItem(), ownersFromItem(), profileName(), STATUSES, today(), AddIssueDialog() (+17 more)
...
Cohesion: 0.09
Nodes (24): calculatedProgress(), INITIATIVE_BUCKETS, initiativeBucket, plannedProgress(), logger, DatabaseService, Injectable, ChartPoint (+16 more)

### Community 86 - "AttachmentDetailPage.tsx"
Cohesion: 0.22
Nodes (10): fmtAed(), ProjectOverviewCards(), Chip(), atRiskSuggested(), calculatedProgress(), MilestoneProgressRow, plannedProgress(), riskScore() (+2 more)
...
## Knowledge Gaps
- **475 isolated node(s):** `CREATE_DEFAULTS`, `Verification gate (all must pass before a phase is ticked)`, `2. Per-session protocol`, `3. Skill mapping`, `Phase 0 — Safety net (prerequisite, no refactoring)` (+470 more)
...
- **Why does `toHttpException()` connect `AddMilestoneDialog.tsx` to `Community 1`, `Community 7`, `Community 9`, `Community 10`, `Community 12`, `Community 13`, `Community 16`, `CreateProjectDto`, `Community 26`, `nest-cli.json`, `Community 34`, `status-reports.repository.ts`, `Community 44`, `nest-cli.json`, `RecordHistoryService`, `@types/node`, `PeopleRepository`, `PaginationQueryDto`, `.add`, `CreateLinkDto`, `WorkflowPanel.tsx`, `WorkflowPanel.tsx`, `AddMilestoneDialog.tsx`, `transform`, `@nestjs/swagger`, `Get`, `eslint-plugin-react-hooks`, `shadcn`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `AuthUser` connect `Community 8` to `Community 1`, `Community 5`, `Community 7`, `Community 10`, `CreateProjectDto`, `status-reports.repository.ts`, `Community 44`, `Community 45`, `RecordHistoryService`, `@nestjs/swagger`, `@types/node`, `PeopleRepository`, `.add`, `CreateLinkDto`, `WorkflowPanel.tsx`, `.add`, `.add`, `WorkflowPanel.tsx`, `README.md`, `lookups.service.ts`, `DatabaseModule`, `ProjectsGrid.tsx`, `eslint-plugin-react-hooks`, `RecordHistoryService`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `CurrentUser` connect `Community 8` to `Community 1`, `Community 5`, `Community 7`, `Community 10`, `CreateProjectDto`, `status-reports.repository.ts`, `Community 44`, `Community 45`, `RecordHistoryService`, `@nestjs/swagger`, `@types/node`, `PeopleRepository`, `.add`, `CreateLinkDto`, `WorkflowPanel.tsx`, `.add`, `.add`, `WorkflowPanel.tsx`, `README.md`, `lookups.service.ts`, `DatabaseModule`, `ProjectsGrid.tsx`, `eslint-plugin-react-hooks`, `RecordHistoryService`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **What connects `CREATE_DEFAULTS`, `Verification gate (all must pass before a phase is ticked)`, `2. Per-session protocol` to the rest of the system?**
