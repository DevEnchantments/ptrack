# Graph Report - ptrack  (2026-07-15)

## Corpus Check
- 149 files · ~48,007 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1288 nodes · 2095 edges · 91 communities (77 shown, 14 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d927b7dc`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Community 0
- Community 1
- Community 2
- Community 3
- Community 4
- Community 5
- Community 6
- Community 7
- Community 8
- Community 9
- Community 10
- Community 11
- Community 12
- Community 13
- Community 14
- Community 15
- Community 16
- CreateProjectDto
- Community 18
- Community 19
- Community 20
- Community 21
- Community 22
- Community 23
- Community 24
- Community 25
- Community 26
- Community 27
- Community 28
- Community 29
- Community 30
- AddMilestoneDialog.tsx
- CreateActionItemDto
- nest-cli.json
- Community 34
- Community 35
- CreateProjectDto
- Community 37
- status-reports.repository.ts
- Community 39
- Community 40
- Community 41
- RecordHistory.tsx
- Community 43
- Community 44
- Community 45
- MiniCalendar.tsx
- Community 47
- Community 48
- toHttpException
- Community 50
- @eslint/js
- transform
- auth-context.tsx
- CLAUDE.md — P-Track
- reflect-metadata
- @types/node
- ActionItemDetailPage.tsx
- PeopleRepository
- StatusReportDetailPage.tsx
- AttachmentsRepository
- .update
- SupabaseAuthGuard
- react-dom
- LinksController
- App.tsx
- CreateIssueDto
- CreateLinkDto
- CreateUpdateDto
- .add
- UpdateProjectDto
- CreateResourceDto
- MilestonesRepository
- CreateStatusReportDto
- PaginationQueryDto
- RecordHistory.tsx
- UpdateAttachmentDto
- AttachmentDetailPage.tsx
- AddPersonDialog.tsx
- MiniCalendar.tsx
- PersonAutocomplete.tsx
- AddIssueDialog.tsx
- .logDeleted
- @eslint/js
- ApiPropertyOptional
- IsOptional
- Type
- status-reports.repository.ts

## God Nodes (most connected - your core abstractions)
1. `AuthUser` - 39 edges
2. `CurrentUser` - 39 edges
3. `toHttpException()` - 38 edges
4. `DatabaseService` - 35 edges
5. `@nestjs/swagger` - 34 edges
6. `cn()` - 34 edges
7. `RecordHistoryService` - 22 edges
8. `compilerOptions` - 22 edges
9. `compilerOptions` - 19 edges
10. `toast` - 17 edges

## Surprising Connections (you probably didn't know these)
- `DialogOverlay()` --calls--> `cn()`  [EXTRACTED]
  frontend/src/components/ui/dialog.tsx → frontend/src/lib/utils.ts
- `DialogDescription()` --calls--> `cn()`  [EXTRACTED]
  frontend/src/components/ui/dialog.tsx → frontend/src/lib/utils.ts
- `ProtectedRoute()` --calls--> `useAuth()`  [EXTRACTED]
  frontend/src/components/ProtectedRoute.tsx → frontend/src/lib/auth-context.tsx
- `Button()` --calls--> `cn()`  [EXTRACTED]
  frontend/src/components/ui/button.tsx → frontend/src/lib/utils.ts
- `CardAction()` --calls--> `cn()`  [EXTRACTED]
  frontend/src/components/ui/card.tsx → frontend/src/lib/utils.ts

## Import Cycles
- None detected.

## Communities (91 total, 14 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.09
Nodes (21): 10. PL/SQL package layer, 11. Notable architectural patterns, 12. Summary, 1. Application overview, 2.1 Project management (core), 2.2 Milestones, 2.3 Action items (tasks), 2.4 Issues (+13 more)

### Community 1 - "Community 1"
Cohesion: 0.09
Nodes (18): ApiConsumes, AttachmentsController, ApiBody, Body, Controller, Delete, Get, Param (+10 more)

### Community 2 - "Community 2"
Cohesion: 0.12
Nodes (13): LookupsController, Body, Controller, Get, Param, Post, LookupsModule, Module (+5 more)

### Community 3 - "Community 3"
Cohesion: 0.12
Nodes (13): MilestonesController, ApiBody, Body, Controller, Delete, Get, Param, Patch (+5 more)

### Community 4 - "Community 4"
Cohesion: 0.04
Nodes (47): devDependencies, eslint, eslint-config-prettier, @eslint/eslintrc, @eslint/js, eslint-plugin-prettier, globals, jest (+39 more)

### Community 5 - "Community 5"
Cohesion: 0.09
Nodes (15): StatusReportsController, ApiBody, Body, Controller, Get, Param, Patch, Post (+7 more)

### Community 6 - "Community 6"
Cohesion: 0.14
Nodes (5): toHttpException(), ActionItemsRepository, Injectable, MilestonesRepository, Injectable

### Community 7 - "Community 7"
Cohesion: 0.09
Nodes (16): IssuesController, ApiBody, Body, Controller, Get, Param, Patch, Post (+8 more)

### Community 8 - "Community 8"
Cohesion: 0.17
Nodes (4): LinksRepository, Injectable, LinksService, Injectable

### Community 9 - "Community 9"
Cohesion: 0.13
Nodes (8): Module, UpdatesModule, Injectable, Update, UpdateListItem, UpdatesRepository, Injectable, UpdatesService

### Community 10 - "Community 10"
Cohesion: 0.16
Nodes (4): ResourcesRepository, Injectable, ResourcesService, Injectable

### Community 11 - "Community 11"
Cohesion: 0.22
Nodes (11): auditLine(), formatLongDate(), MilestoneDetailPage(), MONTHS, ownerLabel(), profileName(), relativeTime(), STATUS_LABELS (+3 more)

### Community 12 - "Community 12"
Cohesion: 0.19
Nodes (8): Controller, Get, Query, UsersController, Module, UsersModule, Injectable, UsersService

### Community 13 - "Community 13"
Cohesion: 0.18
Nodes (20): Card(), CardAction(), CardContent(), CardDescription(), CardFooter(), CardHeader(), CardTitle(), Input() (+12 more)

### Community 14 - "Community 14"
Cohesion: 0.16
Nodes (18): ACTIONS, attachmentUploader(), EXT_STYLES, fileExt(), formatReportDate(), formatSize(), initials(), ISSUE_STATUS_LABELS (+10 more)

### Community 15 - "Community 15"
Cohesion: 0.08
Nodes (25): compilerOptions, allowArbitraryExtensions, allowImportingTsExtensions, erasableSyntaxOnly, jsx, lib, module, moduleDetection (+17 more)

### Community 16 - "Community 16"
Cohesion: 0.11
Nodes (20): App(), ProtectedRoute(), Lookup, lookupsApi, projectsApi, AuthContext, AuthContextValue, AuthProvider() (+12 more)

### Community 17 - "CreateProjectDto"
Cohesion: 0.26
Nodes (5): AuthUser, CurrentUser, Delete, Delete, @nestjs/swagger

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (22): compilerOptions, allowSyntheticDefaultImports, baseUrl, declaration, emitDecoratorMetadata, esModuleInterop, experimentalDecorators, forceConsistentCasingInFileNames (+14 more)

### Community 19 - "Community 19"
Cohesion: 0.09
Nodes (21): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+13 more)

### Community 20 - "Community 20"
Cohesion: 0.12
Nodes (22): Props, AddStatusReportDialog(), EDITABLE_OPTIONS, today(), VIEWABLE_OPTIONS, ConfirmDeleteButton(), Props, Button() (+14 more)

### Community 21 - "Community 21"
Cohesion: 0.10
Nodes (21): eslint-plugin-react-hooks, eslint-plugin-react-refresh, devDependencies, eslint, eslint-plugin-react-hooks, eslint-plugin-react-refresh, globals, @types/node (+13 more)

### Community 22 - "Community 22"
Cohesion: 0.10
Nodes (19): hooks, PreToolUse, permissions, allow, defaultMode, deny, $schema, Bash(git add:*) (+11 more)

### Community 23 - "Community 23"
Cohesion: 0.10
Nodes (19): compilerOptions, allowImportingTsExtensions, erasableSyntaxOnly, lib, module, moduleDetection, noEmit, noFallthroughCasesInSwitch (+11 more)

### Community 24 - "Community 24"
Cohesion: 0.11
Nodes (19): @base-ui/react, class-variance-authority, clsx, dependencies, @base-ui/react, class-variance-authority, clsx, lucide-react (+11 more)

### Community 25 - "Community 25"
Cohesion: 0.10
Nodes (21): dependencies, class-transformer, class-validator, jose, @nestjs/common, @nestjs/core, @nestjs/platform-express, @nestjs/swagger (+13 more)

### Community 26 - "Community 26"
Cohesion: 0.40
Nodes (4): CreateCommentDto, ApiProperty, IsString, MinLength

### Community 27 - "Community 27"
Cohesion: 0.14
Nodes (6): AuthedRequest, DatabaseService, Injectable, Attachment, AttachmentDetail, AttachmentListItem

### Community 28 - "Community 28"
Cohesion: 0.39
Nodes (8): AddActionItemDialog(), emptyOwner(), ownerFromItem(), ownersFromItem(), profileName(), Props, STATUSES, today()

### Community 29 - "Community 29"
Cohesion: 0.12
Nodes (16): jest, collectCoverageFrom, coverageDirectory, moduleFileExtensions, rootDir, testEnvironment, testRegex, transform (+8 more)

### Community 30 - "Community 30"
Cohesion: 0.14
Nodes (14): scripts, build, dev, format, lint, start, start:debug, start:dev (+6 more)

### Community 31 - "AddMilestoneDialog.tsx"
Cohesion: 0.39
Nodes (7): AddMilestoneDialog(), emptyOwner(), ownerFromMilestone(), profileName(), Props, STATUSES, today()

### Community 32 - "CreateActionItemDto"
Cohesion: 0.14
Nodes (11): ActionItemsController, ApiBody, Body, Controller, Delete, Get, Param, Patch (+3 more)

### Community 33 - "nest-cli.json"
Cohesion: 0.15
Nodes (12): ArrayMaxSize, CreateActionItemDto, ApiProperty, ApiPropertyOptional, IsArray, IsDateString, IsIn, IsOptional (+4 more)

### Community 34 - "Community 34"
Cohesion: 0.08
Nodes (33): Props, Props, ActionItem, ActionItemComment, ActionItemOwner, actionItemsApi, apiDelete(), apiGet() (+25 more)

### Community 35 - "Community 35"
Cohesion: 0.20
Nodes (9): name, private, scripts, build, dev, lint, preview, type (+1 more)

### Community 37 - "Community 37"
Cohesion: 0.25
Nodes (7): exclude, extends, dist, node_modules, **/*spec.ts, test, ./tsconfig.json

### Community 38 - "status-reports.repository.ts"
Cohesion: 0.14
Nodes (13): CreateMilestoneDto, ApiProperty, ApiPropertyOptional, IsArray, IsBoolean, IsDateString, IsIn, IsOptional (+5 more)

### Community 39 - "Community 39"
Cohesion: 0.25
Nodes (7): compilerOptions, baseUrl, paths, files, ./src/*, @/*, references

### Community 40 - "Community 40"
Cohesion: 0.22
Nodes (8): @nestjs/config, @nestjs/config, dependencies, @nestjs/config, react-router-dom, @supabase/supabase-js, react-router-dom, @supabase/supabase-js

### Community 41 - "Community 41"
Cohesion: 0.29
Nodes (6): author, description, license, name, private, version

### Community 42 - "RecordHistory.tsx"
Cohesion: 0.19
Nodes (14): CreateProjectDto, ProjectMemberDto, ApiProperty, ApiPropertyOptional, IsArray, IsDateString, IsIn, IsOptional (+6 more)

### Community 43 - "Community 43"
Cohesion: 0.20
Nodes (9): Compile and run the project, Deployment, Description, License, Project setup, Resources, Run tests, Stay in touch (+1 more)

### Community 46 - "MiniCalendar.tsx"
Cohesion: 0.29
Nodes (6): collection, compilerOptions, deleteOutDir, plugins, $schema, sourceRoot

### Community 49 - "toHttpException"
Cohesion: 0.09
Nodes (18): ProjectsController, ApiBody, Body, Controller, Delete, Get, Param, Patch (+10 more)

### Community 54 - "@eslint/js"
Cohesion: 0.23
Nodes (8): HistoryEntry, HistoryInsert, logger, ActionItem, ActionItemComment, ActionItemListItem, Milestone, MilestoneListItem

### Community 55 - "transform"
Cohesion: 0.23
Nodes (7): ApiSecurity, AppController, Controller, Get, AppService, Injectable, Public()

### Community 56 - "auth-context.tsx"
Cohesion: 0.10
Nodes (18): AddResourceDialog(), Props, AddUpdateDialog(), Props, CategorySelect(), Props, EditProjectDialog(), Props (+10 more)

### Community 57 - "CLAUDE.md — P-Track"
Cohesion: 0.18
Nodes (10): CLAUDE.md — P-Track, Current state — Phase 1 complete (full CRUD), graphify, Hard architectural rules (do not violate), How I like to work — follow precisely, Known gotchas — carry these forward, Repo & environment, Roadmap — deferred to Phase 2+ (+2 more)

### Community 58 - "reflect-metadata"
Cohesion: 0.18
Nodes (10): AppModule, Module, ActionItemsModule, Module, PeopleModule, Module, ResourcesModule, Module (+2 more)

### Community 59 - "@types/node"
Cohesion: 0.18
Nodes (10): ApiBody, Body, Controller, Delete, Get, Param, Patch, Post (+2 more)

### Community 60 - "ActionItemDetailPage.tsx"
Cohesion: 0.50
Nodes (3): Expanding the ESLint configuration, React Compiler, React + TypeScript + Vite

### Community 61 - "PeopleRepository"
Cohesion: 0.07
Nodes (26): CreatePersonDto, ApiProperty, ApiPropertyOptional, IsIn, IsOptional, IsString, IsUUID, MaxLength (+18 more)

### Community 62 - "StatusReportDetailPage.tsx"
Cohesion: 0.22
Nodes (9): ResourcesController, ApiBody, Body, Controller, Delete, Get, Param, Patch (+1 more)

### Community 64 - ".update"
Cohesion: 0.18
Nodes (10): 1 · Database — Supabase SQL editor, 2 · Backend — `:3000`, 3 · Frontend — `:5173`, 4 · Explore, 🏗 Architecture, 📚 More docs, 🧪 Quality, 🚀 Quick start (+2 more)

### Community 67 - "LinksController"
Cohesion: 0.20
Nodes (9): LinksController, ApiBody, Body, Controller, Delete, Get, Param, Patch (+1 more)

### Community 68 - "App.tsx"
Cohesion: 0.21
Nodes (10): ActionItemDetailPage(), commentAuthor(), ownersLabel(), STATUS_LABELS, Tab, TABS, ACCESS_LABELS, authorName() (+2 more)

### Community 69 - "CreateIssueDto"
Cohesion: 0.18
Nodes (10): CreateIssueDto, ApiProperty, ApiPropertyOptional, IsArray, IsIn, IsOptional, IsString, IsUUID (+2 more)

### Community 70 - "CreateLinkDto"
Cohesion: 0.18
Nodes (10): CreateLinkDto, ApiProperty, ApiPropertyOptional, IsArray, IsBoolean, IsOptional, IsString, MaxLength (+2 more)

### Community 71 - "CreateUpdateDto"
Cohesion: 0.20
Nodes (9): CreateUpdateDto, ApiProperty, ApiPropertyOptional, IsArray, IsBoolean, IsOptional, IsString, IsUUID (+1 more)

### Community 72 - ".add"
Cohesion: 0.21
Nodes (4): RecordHistoryService, Injectable, Owners, ownersLabel()

### Community 73 - "UpdateProjectDto"
Cohesion: 0.20
Nodes (9): ApiPropertyOptional, IsArray, IsDateString, IsIn, IsOptional, IsString, IsUUID, MaxLength (+1 more)

### Community 74 - "CreateResourceDto"
Cohesion: 0.22
Nodes (8): CreateResourceDto, ApiProperty, ApiPropertyOptional, IsOptional, IsString, IsUUID, MaxLength, UpdateResourceDto

### Community 75 - "MilestonesRepository"
Cohesion: 0.40
Nodes (4): LinksModule, Module, Link, LinkListItem

### Community 76 - "CreateStatusReportDto"
Cohesion: 0.25
Nodes (7): CreateStatusReportDto, ApiProperty, IsDateString, IsIn, IsString, MaxLength, UpdateStatusReportDto

### Community 77 - "PaginationQueryDto"
Cohesion: 0.29
Nodes (7): ApiPropertyOptional, PaginationQueryDto, IsInt, IsOptional, Max, Min, Type

### Community 78 - "RecordHistory.tsx"
Cohesion: 0.39
Nodes (6): initials(), Props, RecordHistory(), relativeTime(), username(), HistoryEntry

### Community 79 - "UpdateAttachmentDto"
Cohesion: 0.29
Nodes (6): ApiPropertyOptional, IsArray, IsBoolean, IsOptional, IsString, UpdateAttachmentDto

### Community 80 - "AttachmentDetailPage.tsx"
Cohesion: 0.48
Nodes (5): AttachmentDetailPage(), formatSize(), longDate(), relativeTime(), uploaderName()

### Community 81 - "AddPersonDialog.tsx"
Cohesion: 0.47
Nodes (5): ACCESS_LEVELS, AddPersonDialog(), emptyPerson(), memberName(), Props

### Community 82 - "MiniCalendar.tsx"
Cohesion: 0.40
Nodes (5): dateParts(), MiniCalendar(), MONTHS, Props, WEEKDAY_HEADERS

### Community 83 - "PersonAutocomplete.tsx"
Cohesion: 0.40
Nodes (4): Props, usersApi, UserSummary, ProjectMemberInput

### Community 84 - "AddIssueDialog.tsx"
Cohesion: 0.50
Nodes (3): AddIssueDialog(), emptyPerson(), Props

### Community 85 - ".logDeleted"
Cohesion: 0.67
Nodes (3): DatabaseModule, Global, Module

## Knowledge Gaps
- **316 isolated node(s):** `name`, `version`, `description`, `author`, `private` (+311 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **14 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `@nestjs/swagger` connect `CreateProjectDto` to `nest-cli.json`, `CreateIssueDto`, `CreateLinkDto`, `status-reports.repository.ts`, `Community 5`, `UpdateProjectDto`, `RecordHistory.tsx`, `CreateResourceDto`, `CreateStatusReportDto`, `CreateUpdateDto`, `MiniCalendar.tsx`, `UpdateAttachmentDto`, `transform`, `Community 26`, `PeopleRepository`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `AuthUser` connect `CreateProjectDto` to `CreateActionItemDto`, `Community 1`, `LinksController`, `Community 3`, `Community 5`, `Community 7`, `toHttpException`, `@types/node`, `PeopleRepository`, `StatusReportDetailPage.tsx`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `CurrentUser` connect `CreateProjectDto` to `CreateActionItemDto`, `Community 1`, `LinksController`, `Community 3`, `Community 5`, `Community 7`, `toHttpException`, `@types/node`, `PeopleRepository`, `StatusReportDetailPage.tsx`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **What connects `name`, `version`, `description` to the rest of the system?**
  _316 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.09090909090909091 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.08912655971479501 - nodes in this community are weakly interconnected._
- **Should `Community 2` be split into smaller, more focused modules?**
  _Cohesion score 0.11594202898550725 - nodes in this community are weakly interconnected._