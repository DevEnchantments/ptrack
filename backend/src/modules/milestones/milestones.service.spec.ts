...
describe('MilestonesService', () => {
  ...
  function build(existing: unknown = { id: MILESTONE, name: 'Cutover' }) {
    const mocks = {
      ...
      adjustWeights: jest.fn().mockResolvedValue(undefined),
  ...
  describe('adjustWeights', () => {
    it('rejects a set that does not total exactly 100', async () => {
      ...
      expect(mocks.adjustWeights).not.toHaveBeenCalled();
    });

    it('accepts a set that totals 100, applying it as one atomic RPC', async () => {
      // main diverges from the branch here (deliberately): weights go through
      // repo.adjustWeights -> db/adjust_milestone_weights.sql, not N updates.
      const { service, mocks } = build();
      ...
      expect(mocks.adjustWeights).toHaveBeenCalledWith(
        PROJECT,
        ['a', 'b'],
        [60, 40],
        USER,
