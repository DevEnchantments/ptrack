...
| F6 | ~~OPEN~~ **CLOSED, see C7** | `frontend/tsconfig.json` had a deprecated `baseUrl` | A bare `npx tsc --noEmit` in `frontend/` fails with TS5101. The build is unaffected because it uses `tsc -b`, which is why it has survived this long. |
