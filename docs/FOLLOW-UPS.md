...
| --- | --- | --- | --- |
| F2 | ~~BLOCKED~~ **CLOSED, see C10** | The `attachments` table has **no `updated_by` or `updated_at` column at all** (its columns end at `uploaded_by, created_at`) | Originally logged as "the service forgets to stamp `updated_by`". Checking before fixing showed there is nothing to stamp: the table records who uploaded a file and never who edited it afterwards. So this is a schema gap, not a service oversight, and it needs a migration plus a decision about whether attachment edits are worth auditing at all. |
| F3 | ~~OPEN~~ **CLOSED, see C11** | `submissions.reject` used to stamp `returned_by` / `returned_at` | A rejection writes the *returned* columns, so "who rejected this, and when" is not recoverable from the audit columns. Likely because the schema has no `rejected_*` columns; needs a schema decision, not a code tweak. |
...
| F5 | ~~OPEN~~ **CLOSED, see C12** | Program-outcome numbering could still collide on **concurrent** creates | `max + 1` fixed the deletion-gap collision, but two simultaneous creates still race. A real fix needs a unique constraint or sequence, i.e. a schema change. |
| F6 | ~~OPEN~~ **CLOSED, see C7** (with a correction, see L6) | `frontend/tsconfig.json` had a deprecated `baseUrl` | A bare `npx tsc --noEmit` in `frontend/` fails with TS5101. The build is unaffected because it uses `tsc -b`, which is why it has survived this long. |
...
| F8 | OPEN — **4 left** | **10 of 25 modules had no repository**, running Supabase queries directly inside services. Six were fixed on 2026-08-19 (`access-admin`, `lookups`, `registry`, `search`, `reports`, `dashboard`); **`import`, `project-sections`, `templates` and `users` still have none** | CLAUDE.md rule 3 describes 60% of the backend, not all of it. This is also **why those modules are untested**: with no seam, the only testable thing is a mocked Supabase chain, which asserts on the fake rather than the code. The missing tests are the symptom; the missing seam is the cause. Extracting a repository is therefore part of Phase 0, not a separate refactor. |
...
| L6 | ACCEPTED | **A bare `npx tsc --noEmit` in `frontend/` checks nothing.** The root tsconfig has `"files": []` with project references, so the command exits 0 having examined no files - it reported success on genuinely broken syntax on 2026-08-20. Removing the deprecated `baseUrl` (C7) silenced TS5101 but did not make the command useful, and the claim in that entry that "the typecheck is usable outside `tsc -b`" was wrong. **The frontend's only real typecheck is `npm run build`.** |
...
| C12 | 2026-08-20 | **Program-outcome numbering could still collide on concurrent creates** (was F5). `db/program_outcome_numbering.sql` adds a partial unique index on `(project_id, sort_order)`, which is the only place the race can actually be settled; the service retries an auto-assigned number up to three times, re-reading each attempt. A caller-supplied number is not retried — their collision is a 409 they should see. **Run the SQL.** |
| C11 | 2026-08-20 | **A rejection was indistinguishable from a return in the audit trail** (was F3). `db/submission_rejection_columns.sql` adds `rejected_by` / `rejected_at`, and the workflow table's reject row now stamps its own pair. Historical rows keep their `returned_at`: backfilling would invent a trail that was never recorded. **Run the SQL.** |
| C10 | 2026-08-20 | **Attachments recorded who uploaded a file but never who edited it** (was F2). `db/attachment_audit_columns.sql` adds `updated_by` / `updated_at`, backfilled from `uploaded_by` for existing rows, and `AttachmentsService.update()` now takes the actor and stamps both. **Run the SQL.** |
...
| D4 | The last-admin guard tests `newRole === 'admin'`, which looks like a real branch | Unreachable. `updateRole` returns early when the role is unchanged, so by the time the guard runs, a current role of `admin` implies the new role is not `admin`. A mutation check proved deleting the condition changes no test and no behaviour. **Kept anyway**, and commented: now that the guard is its own function it should hold on its own terms rather than depend on the order of checks in today's only caller. Not every dead branch is worth deleting. |
