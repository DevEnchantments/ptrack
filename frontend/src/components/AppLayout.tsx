...
export function AppLayout() {
  ...
  // Projects is the only live section, so it owns "/" and every project route.
  ...
  return (
    <div className="flex min-h-svh">
      <aside
        className={`flex shrink-0 flex-col bg-neutral-950 text-neutral-300 transition-all ${
          collapsed ? 'w-14' : 'w-56'
        }`}
      >
          ...
          {NAV_ITEMS.map((item) =>
            item.to ? (
              <NavLink
                ...
                className={`flex items-center gap-3 rounded-md px-2.5 py-2 text-sm hover:bg-neutral-800 hover:text-white ${
                  projectsActive
              ...
              <span
                ...
                className="flex cursor-not-allowed items-center gap-3 rounded-md px-2.5 py-2 text-sm text-neutral-600"
              >
                <item.icon className="h-4 w-4 shrink-0" />
                {!collapsed && <span className="truncate">{item.label}</span>}
              </span>
